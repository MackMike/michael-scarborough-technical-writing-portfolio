# Michael Scarborough Technical Writing Portfolio

Welcome to my technical writing portfolio.

This repository contains writing, editing, and documentation samples that demonstrate my experience in technical communication, technical editing, structured content, and administrator-focused documentation. My background includes macOS systems engineering, enterprise IT documentation, and user-focused technical content.

## Portfolio Contents

### Writing and Documentation Samples
- [Managed Mac Onboarding Guide](./Managed-Mac-Onboarding-Guide/)
- [KB: Troubleshooting Compliance Failures on Jamf-Managed Macs](./Jamf-Managed-Mac-Compliance-Troubleshooting/)
- [KB: Troubleshooting VPN Certificate Issues on Managed macOS Devices](./VPN-Certificate-Managed-Mac-Troubleshooting/)
- [Migrate-User-Administrative-Tool-Documentation](./Migrate-User-Administrative-Tool-Documentation/)
  [DITA-Sample-Managed-Mac-Onboarding-Guide](./DITA-Sample-Managed-Mac-Onboarding-Guide/)
### Additional Samples
- [OpenOffice Technical Editing](./Apache-OpenOffice-Technical-Editing/)
- [Atari Computer User Guide](./Atari-Computer-User-Guide/)

## About Me
I am a technical writing and editing professional with a background in macOS systems engineering and enterprise IT. My work includes technical editing, administrator documentation, knowledge base content, workflows, and instructional documentation for technical and nontechnical audiences.

## Skills Demonstrated
This portfolio includes examples of:

- Technical writing
- Technical editing
- User guides
- Knowledge base documentation
- Administrator-focused documentation
- Process and workflow documentation
- Markdown
- Structured authoring fundamentals

## Additional Links
- [LinkedIn](https://www.linkedin.com/in/miscarborough/)

## Notes
Additional samples may be added over time as I continue expanding this portfolio.
